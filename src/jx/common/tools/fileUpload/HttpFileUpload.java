...
{
    // 有默认参数文件
    public static ResourceBundle config = ResourceBundle.getBundle("log4j");
    ...
    private static String realSavePath = config.getString("realSavePath");//保存在磁盘下的路径:绝对路径;
    private static boolean isSuc = false;

    public static void main(String[] args){
    	Locale locale = Locale.getDefault();
    	ResourceBundle configx = ResourceBundle.getBundle("config",locale);
    }
     ...
     */
    private void setAllowFileTypes(String allowFileTypes)
    {
        this.allowFileTypes = allowFileTypes;
    }

    private void addAllowFileTypes(String allowFileTypes)
    {
        this.allowFileTypes += ("," + allowFileTypes);
    }
    ...
    public List parseRequest(HttpServletRequest request) throws FileUploadException{
    	try {
			...
			throw new FileUploadException("设置编码错误!");
		}
        ...
        if (requestSize == -1)
        {
            throw new UnknownSizeException("未知文件!");
        }
        long sizeMax = getSizeMax();
        
        if (sizeMax >= 0 && requestSize > sizeMax)
        {
            throw new SizeLimitExceededException("文件总大小'" + requestSize/1048576
                    + "M'超过'" + sizeMax/1048576 + "M'", requestSize,
                sizeMax);
        }
                    ...
                    else
                    {
                        ...
                        // 文件域
                        {
                            ...
                            if (fileSize > this.fileLimitSize)
                            {
                                ...
                                throw new InvalidFileUploadException("请上传'"+
                                        this.fileLimitSize/1048576 + "M'以下大小的文件!",
                                    invalidFiles);
                            }
                            items.add(item);
                        }
	...
	public static String saveRelFile(FileItem fileItm) throws Exception
	{
		File upFile = new File(getRealSavePath()+getFilePathName(fileItm.getName()));
		
		if(!upFile.getParentFile().exists())
			upFile.getParentFile().mkdir();
		
		fileItm.write(upFile);
