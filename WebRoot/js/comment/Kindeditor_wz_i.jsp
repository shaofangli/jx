	...
	<script type="text/javascript">
		...
		updatewzcz = function(){
			...
			if(!$('bt').value || trim($('bt').value)==''){
				myAlert('提示信息','标题不能为空！',true,'i_wznr_form');
				return false;
			}
			
			if(!$('content1').value){
				myAlert('提示信息','文章内容不能为空！',true,'i_wznr_form');
				return false;
			}
			
			myAlert('提示信息','正在保存,请稍候...',false,'i_wznr_form');
			new Ajax.Request('<c:url value="/manage/wzgl_ajax.htm"/>',
	                {
	                	method:'post',
	                	asynchronous:true,
	                	evalScripts : true,
	                	parameters:'method=insertAction&'+Form.serialize('i_wznr_form'),
...
<body bgcolor="white">
	...
	<form id="i_wznr_form" name="i_wznr_form">
		...
		<input type="button" id="subbut" class="addbut" onclick="updatewzcz()"/> (换行: Shift + Enter,换段: Enter)
	</form>
	</div>
	<script type="text/javascript">
		$('lx').value= window.parent.document.forms[0].dh.value;
	</script>
</body>
</html>
