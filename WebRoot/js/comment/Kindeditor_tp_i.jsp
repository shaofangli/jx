	...
	<script type="text/javascript">
		...
		inserttpcz = function(){
			...
			if(!$('bt').value || trim($('bt').value)==''){
				myAlert('提示信息','图片标题不能为空！',true,'i_tpnr_form');
				return false;
			}
			
			if(!$('content1').value){
				myAlert('提示信息','图片内容不能为空！',true,'i_tpnr_form');
				return false;
			}
			
			myAlert('提示信息','正在保存,请稍候...',false,'i_tpnr_form');
			new Ajax.Request('<c:url value="/manage/tpgl_ajax.htm"/>',
	                {
	                	method:'post',
	                	asynchronous:true,
	                	evalScripts : true,
	                	parameters:'method=insertAction&'+Form.serialize('i_tpnr_form'),
...
<body>
	...
	<form id="i_tpnr_form" name="i_tpnr_form">
		...
		<input type="button" id="subbut" class="addbut" onclick="inserttpcz()"/> (换行: Shift + Enter,换段: Enter)
	</form>
	</div>
	<script type="text/javascript">
			$('lx').value= window.parent.document.getElementById('lxdh').value;
	</script>
</body>
</html>
